...
<template>
	<uni-section title="打卡情况" type="line" style="background-color:#d0d0d0;"></uni-section>
			...
			<view class="layout">
				<text class="layText" v-if="info.message.length!==0">上班打卡时间:{{info.message[0].timeIn}}</text>
				<text class="layText" v-if="info.message.length!==0">下班打卡时间:{{info.message[0].timeOut}}</text>
				...
				<text class="timeNow" v-if="isToday(data.data.date)">现在时间:{{timeNow}}</text>
				...
				<button v-if="isToday(data.data.date)" @click="changePage2">打卡</button>
				<navigator url="">
						...
						success: (res) => {
							this.info.message = res.data.data.records.map(record => {
								...
								const timeOut=new Date(record.timeOut)
							        return {
							            "timeIn":timeIn.getFullYear()+"年"+(timeIn.getMonth()+1)+"月"+timeIn.getDate()+"日"+timeIn.getHours()+":"+timeIn.getMinutes()+":"+timeIn.getSeconds(),
							            "timeOut": timeOut.getFullYear()+"年"+(timeOut.getMonth()+1)+"月"+timeOut.getDate()+"日"+timeOut.getHours()+":"+timeOut.getMinutes()+":"+timeIn.getSeconds()
							        }
							    }
								});
						},
